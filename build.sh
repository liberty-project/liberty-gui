...
if [ "$platform" == "darwin" ]; then
    BIN_PATH=$BIN_PATH/naos-wallet-gui.app/Contents/MacOS/
elif [ "$platform" == "mingw64" ] || [ "$platform" == "mingw32" ]; then
    NAOSD_EXEC=naosd.exe
fi
...
get_tag
popd
echo "var GUI_NAOS_VERSION = \"$TAGNAME\"" >> version.js
...
$QMAKE ../naos-wallet-gui.pro "$CONFIG" || exit
$MAKE || exit 

# Copy naosd to bin folder
if [ "$platform" != "mingw32" ] && [ "$ANDROID" != true ]; then
cp ../$NAOS_DIR/bin/$NAOSD_EXEC $BIN_PATH
fi
