...
{
    // prepare command line arguments and pass to naosd
    ...
    // Start naosd
    bool started = m_daemon->startDetached(m_naosd, arguments);
    ...
    while(true && !m_app_exit) {
        ...
        if(running(nettype)) {
            ...
            if(counter >= 5) {
                qDebug() << "Killing it! ";
#ifdef Q_OS_WIN
                QProcess::execute("taskkill /F /IM naosd.exe");
#else
                QProcess::execute("pkill naosd");
#endif
            }
...
{ 
    ...
    qDebug() << status;
    // `./naosd status` returns BUSY when syncing.
    ...
    : QObject(parent)
{
    ...
    // Platform depetent path to naosd
#ifdef Q_OS_WIN
    m_naosd = QApplication::applicationDirPath() + "/naosd.exe";
#elif defined(Q_OS_UNIX)
    m_naosd = QApplication::applicationDirPath() + "/naosd";
#endif
